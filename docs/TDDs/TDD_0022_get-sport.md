...
## Listado de Deportes:
- Endpoint Listado: `GET /api/v1/deportes`
- Request Body (UpdateMemberRequest):
## Detalle de Deporte:
- Endpoint Detalle: `GET /api/v1/deportes/:id`
- Request Body (UpdateMemberRequest):
...
5. **Adaptador de Entrada**: `SportController` (Rutas `GET /api/v1/deportes` y `GET /api/v1/deportes/:id` que devuelven status 200).

## Casos de Borde y Errores

| Escenario                     | Resultado Esperado                            | Código HTTP actual        |
| ----------------------------- | --------------------------------------------- | ------------------------- |
| Deporte inexistente (por ID)  | Mensaje: "No existe deporte con ese id"       | 400 Bad Request           |
| Deporte dado de baja (por ID) | Mensaje: "No existe deporte con ese id"       | 400 Bad Request           |
...
3. Exponer las rutas `GET /api/v1/deportes` y `GET /api/v1/deportes/:id` en el `SportController` y registrarlas en la app de Fastify.
